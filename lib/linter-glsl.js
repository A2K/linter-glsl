...
const verboseRegex = /^(.*(?:\.|_))(vertex|fragment)(\.glsl)$/
...
const extractShaderFilenameTokens = shaderFilename => {
  ...
  const extVerboseMatch = verboseRegex.exec(fileName);
  ...
  } else if (extVerboseMatch) {
    baseFilename = extVerboseMatch[1];
    baseShaderType = shaderByChar4(extVerboseMatch[2].substr(0, 4));
    baseExtension = extVerboseMatch[3];
  } else {
...
export default {
  ...
  activate: _ => {
    ...
    this.subscriptions.add(
      atom.config.observe('linter-glsl.glslangValidatorArgs', (glslangValidatorArgs) => {
        this.glslangValidatorArgs = glslangValidatorArgs;
      }));
  },
  ...
  provideLinter: _ => {
    ...
    return {
      ...
      lint: (activeEditor) => {
        ...
        if (this.glslangValidatorArgs) {
            args = this.glslangValidatorArgs.match(/(?:[^\s"]+|"[^"]*")+/g).map(arg => arg.replace(/\"/g, ''));
        }
